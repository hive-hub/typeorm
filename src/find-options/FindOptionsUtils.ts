...
export class FindOptionsUtils {
     ...
     */
    static applyFindManyOptionsOrConditionsToQueryBuilder<
     ...
     */
    static applyOptionsToQueryBuilder<T extends ObjectLiteral>(
        qb: SelectQueryBuilder<T>,
        ...
        if (options.select) {
            ...
            options.select.forEach((select) => {
                if (!metadata.hasColumnWithPropertyPath(`${select as string}`))
                    throw new TypeORMError(
                        `${select as string} column was not found in the ${
                            metadata.name
                        } entity.`
                    );
        ...
        if (options.lock) {
            if (options.lock.mode === "optimistic") {
                ...
                const tableNames = options.lock.tables
                    ? options.lock.tables.map((table) => {
                          const tableAlias = qb.expressionMap.aliases.find(
                              (alias) => {
                                  return (
