    ...
    it("should be able to save and remove entities of different types", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                await connection.manager.findOne(Post, 1).should.eventually.be
                    .undefined;
                await connection.manager.findOne(Category, 1).should.eventually
                    .be.undefined;
                await connection.manager.findOne(User, 1).should.eventually.be
                    .undefined;
            })
        ));
});
